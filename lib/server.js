...
const allowedTemplates = {
    facebook: { text: "text", image: "image", video: "video", audio: "audio", button: "button", sticker: "sticker", carousel: "carousel" },
...
const sendReactApp = mut_response => mut_body => () => {
    ...
    mut_response.setHeader('Content-Type', 'text/html');
    setXPowByHeader(mut_response);
    log(`${baseDir}/frontend/build/index.html`);
    fs.createReadStream(`${baseDir}/frontend/build/index.html`).pipe(mut_response);
};
...
const sendStaticAssets = pathname => mut_response => mut_body => () => {
    ...
    // todo:handle error
    fs.createReadStream(`${baseDir}/frontend/build${pathname}`).pipe(mut_response);
};
...
const deleteScenarios = (pathname) => mut_response => mut_body => () => {
    ...
    if (!/scenario_\S/.test(scenarioID)) return reqInputErrHandler(mut_response)()(`scenario id is invalid`);
